...
int InitFeature( CvSize imgSize )
{
	...
	sampleIntv = 4; // NOTE!!! the distance(row or col) between two adjacent sample points is sampleIntv
...
void GetFeature( CvArr *faceImg32, CvMat *ft32 )
{
	...
	float *pSrc, *pDst = (float *)(ft32->data.ptr);
	float *pBase = (float *)(tPicF->data.ptr);
	float f1, f2;
	int dstStep = ft32->step / sizeof(float);
/*	
	CString tt;
	tt.Format("dst Step = %d", dstStep);
	MessageBox1(tt);
*/
	for (int i = 0; i < scaleNum; i++)
	{
