...
namespace DollarRecognizer
{
	...
	void GeometricRecognizer::loadTemplates()
	{
		...
		addTemplate("Z", samples.getGestureZ());
		addTemplate("~", samples.getGestureHighRising());
		addTemplate("J", samples.getGestureJ());
		addTemplate("Falling", samples.getGestureFalling());
		addTemplate("Rising", samples.getGestureRising());
		addTemplate("Falling Rising", samples.getGestureFallingRising());
		addTemplate("Gesture Marker", samples.getGestureMarker());
	...
	{
		double distance = 0;
		for (int i = OFFSET; i < (int)points.size(); i++)
	...
	Path2D GeometricRecognizer::resample(Path2D points)
	{
		...
		newPoints.push_back(points[OFFSET]);
	    for(int i = OFFSET; i < (int)points.size(); i++)
		{
	...
	{
		//Point2D c     = Point2D(0, 0);//centroid(points);
		//char* tt = "";
		//sprintf(tt, "x = %d, y = %d/n", c.x, c.y);
		//TRACE(tt);
		...
		{
			Point2D point = *i;
			double qx = point.x*cosine - point.y*sine;
			double qy = point.x*sine   + point.y*cosine;
