...
int InitFeature( CvSize imgSize )
{
	...
	blockNum = Size(4,10); // 10row-by-4col blocks each face for blockwise FLD. 
	...
	winNum = Size(4,1); // 1row-by-4col windows each block for histogramming
	lgbpBinNum = 8; // quantize 256 LBP values to lgbpBinNum levels
...
void GetFeature( CvArr *faceImg32, CvMat *ft32 )// ft32 could be 8UC1, but for compatibility, set to 32F
{
	...
	for (int gr = 0; gr < gKerNum.height; gr++)
	{
		for (int gc = 0; gc < gKerNum.width; gc++)
		{
			...
			//DispCvArr(&IplImage(lgbpFace),"lgbp Face");
	...
	ft.copyTo(Mat(ft32));
//	DispCvArr(&IplImage(tmpFt),"feature");
}
...
void GetLgbpHist( Mat & lgbpFace, Mat & ft1 ) 
{
	...
	uchar val;
	uchar *pSrc = lgbpFace.data;
	float *pDst = (float*)(ft1.data);
	int bidx = 0, widx;
				...
				for (int wc = 0; wc < winNum.width; wc++)
				{
					for (int r = 0; r < winSz.height; r++)
					{
						for (int c = 0; c < winSz.width; c++)
						{
							...
							(*(pDst + bidx * ft1.step[0] / sizeof(float) + lgbpBinNum*widx + val))++;
						}
					}
					widx++;
				}
